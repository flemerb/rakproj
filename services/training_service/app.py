...
REQUEST_COUNT = Counter(
    'training_service_requests_total',
    'Total requests to Training Service',
    ['method', 'endpoint', 'status']
)
REQUEST_LATENCY = Histogram(
    'training_service_request_duration_seconds',
    'Request latency in seconds',
    ['method', 'endpoint']
)
TRAINING_ACTIVE = Gauge(
    'training_active',
    'Whether model training is currently active (1) or not (0)'
)
    ...
    try:
        ...
        with mlflow.start_run(run_name="lstm_training") as run:
            ...
            if history and hasattr(history, 'history'):
                ...
                TRAINING_LOSS.labels(split='train').set(final_loss)
                TRAINING_LOSS.labels(split='validation').set(val_loss)
                TRAINING_ACCURACY.labels(split='train').set(final_acc)
                TRAINING_ACCURACY.labels(split='validation').set(val_acc)
            ...
            mlflow.log_artifact(f"{MODELS_PATH}/mapper.json")
